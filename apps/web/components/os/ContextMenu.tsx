...
export const ContextMenu = () => {
    ...
    const adjustedY = typeof window !== 'undefined' && y + items.length * 40 > window.innerHeight ? y - (items.length * 40) : y;
            ...
            {isOpen && (
              ...
              `}
                        >
                            ...
                            {item.shortcut && <span className="text-xs text-[rgba(var(--accent-color),0.3)] font-mono">{item.shortcut}</span>}
                        </motion.button>
                    ))}
                </motion.div>
            )}
