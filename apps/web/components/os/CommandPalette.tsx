...
import { Search, User, FileText, FolderGit2, Mail, Terminal as TerminalIcon, FolderOpen, Bot, Monitor, X } from "lucide-react";
...
export const CommandPalette = () => {
    ...
    return (
        <div className="fixed inset-0 z-[99999] bg-black/60 backdrop-blur-sm flex items-start justify-center pt-[20vh]" onClick={() => setOpen(false)}>
            <div
            ...
            >
                <div className="flex items-center border-b border-[rgba(var(--accent-color),0.2)] px-3 py-2">
                    ...
                    />
                    <button onClick={() => setOpen(false)} className="ml-2 text-white/50 hover:text-white">
                        <X className="h-4 w-4" />
                    </button>
                </div>
                ...
                <div className="max-h-[300px] overflow-y-auto p-2 custom-scrollbar">
                    {filteredCommands.length === 0 ? (
                        <div className="py-6 text-center text-sm text-white/50">No results found.</div>
                    ) : (
                        <div className="space-y-1">
                            <div className="text-xs font-medium text-white/50 px-2 py-1.5 uppercase tracking-wider">Apps & Commands</div>
                            {filteredCommands.map((cmd, index) => (
                                <button
                                    ...
                                    className={`w-full flex items-center rounded-sm px-2 py-1.5 text-sm outline-none transition-colors ${index === selectedIndex ? "bg-[rgba(var(--accent-color),0.2)] text-[rgb(var(--accent-color))]" : "text-[rgba(var(--accent-color),0.8)] hover:bg-[rgba(var(--accent-color),0.1)]"
                                        }`}
                                >
