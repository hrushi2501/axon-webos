...
export const TaskManager = () => {
  ...
  const { stats, metricsSource } = useSocketStore();
  ...
  const sourceLabel =
    metricsSource === "live"
      ...
      : metricsSource === "simulated"
        ? "Demo telemetry (API unavailable)"
  ...
  return (
    <div className="flex flex-col h-full bg-black/40 backdrop-blur-xl text-white">
        ...
        <div className="bg-white/5 p-4 rounded-xl border border-white/10 flex items-center gap-4">
          ...
          <div>
            ...
            <div className="text-2xl font-bold font-mono">
              {stats ? `${stats.cpuUsagePercent.toFixed(1)}%` : "Unavailable"}
            </div>
          </div>
        </div>
        <div className="bg-white/5 p-4 rounded-xl border border-white/10 flex items-center gap-4">
          ...
          <div>
            ...
            <div className="text-2xl font-bold font-mono">
              {stats ? `${memPercentage.toFixed(1)}%` : "Unavailable"}
            </div>
            <div className="text-xs text-white/40 mt-1">
              {stats
                ? `${formatMemory(stats.memoryUsedBytes)} GB / ${formatMemory(stats.memoryTotalBytes)} GB`
                : ""}
            </div>
          </div>
        </div>
      ...
      <div className="px-4 py-2 text-xs border-b border-white/10 text-white/50">
        {sourceLabel}
      </div>
        ...
        <table className="w-full text-left border-collapse">
          ...
          <tbody className="text-sm">
            {Object.values(windows).map((win) => {
              ...
              return (
                ...
                >
                  ...
                  <td className="py-3 text-[rgb(var(--accent-color))]">
                    Running
                  </td>
            ...
            <tr className="border-b border-white/5 hover:bg-white/5 transition-colors opacity-60">
              ...
              <td className="py-3 text-[rgba(var(--accent-color),0.7)]">
                System
              </td>
              <td className="py-3 text-white/60 font-mono">
                {stats ? formatMemoryMB(stats.memoryUsedBytes * 0.2) : "0"} MB
              </td>
            ...
            <tr className="border-b border-white/5 hover:bg-white/5 transition-colors opacity-60">
              ...
              <td className="py-3 text-white/60 font-mono">
                {stats ? formatMemoryMB(stats.memoryUsedBytes * 0.1) : "0"} MB
              </td>
              <td className="py-3 text-right pr-2"></td>
            </tr>
          </tbody>
