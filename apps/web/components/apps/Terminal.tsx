...
export const Terminal = () => {
    ...
    useEffect(() => {
        ...
        bootLines.forEach((line, index) => {
            ...
            setTimeout(() => {
                setHistory(prev => [...prev, { id: Date.now() + Math.random().toString(), text: line, color: "text-gray-300" }]);
            }, delay);
        });
    }, []);
    ...
    const { sendTerminalCommand, registerTerminalCallback, unregisterTerminalCallback } = useSocketStore();

    useEffect(() => {
        registerTerminalCallback((output) => {
            setHistory(prev => [...prev, { id: Date.now().toString(), text: output }]);
        });
        return () => unregisterTerminalCallback();
    }, [registerTerminalCallback, unregisterTerminalCallback]);
    ...
    const handleCommand = async (cmdStr: string) => {
        ...
        if (commands[commandName]) {
            ...
            // Fallback to backend or not found
            // For now, let's treat unknown as "send to backend" or "not found"
            // Since we want client-side feel, let's look for backend only if explicitly intended?
            // Actually, the prompt said "make them all work and proper like linux". 
            // If we don't have it locally, maybe send to backend socket?
            // But existing socket was for backend terminal. Let's keep that connection alive.

            sendTerminalCommand(cmdStr);
        }
    };
    ...
    return (
        ...
        >
            ...
            <div className="flex-1 overflow-auto space-y-1 pb-2 relative z-0 scrollbar-hide">
                {history.map((item) => (
                    <div key={item.id} className={`${item.color || 'text-gray-300'} whitespace-pre-wrap break-words`}>
                        {item.isCommand ? (
                            <div className="flex gap-2">
                                <span className="text-green-400">guest@axon-os:{item.path || "~"}$</span>
