...
import type { AppLaunchData, OpenWindowOptions } from "@/types/os";
...
export const useWindowStore = create<WindowStore>()((set, get) => ({
  ...
  openWindow: (appId, options) => {
    ...
    // If a window already exists, focus it and accept new launch data (for example a File Manager path).
    ...
    if (existingWindow) {
      set({
        windows: {
          ...
          [id]: {
            ...existingWindow,
            title: options?.title || existingWindow.title,
            initialData: options?.data ?? existingWindow.initialData,
          },
        },
      });
      get().focusWindow(id);
    ...
    const newWindow: WindowState = {
      ...
      isMaximized: false,
      position: {
        x: 50 + windowOrder.length * 20,
        y: 50 + windowOrder.length * 20,
      },
