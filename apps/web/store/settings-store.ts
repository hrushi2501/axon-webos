...
export const WALLPAPERS = [
    { id: 'default', name: 'Axon Default', value: 'radial-gradient(circle at center, #1a1a1a 0%, #000000 100%)' },
    { id: 'neon-sunset', name: 'Neon Sunset', value: 'linear-gradient(to bottom right, #2d1b4e, #1c1c1c)' },
    { id: 'cyber-grid', name: 'Cyber Grid', value: 'linear-gradient(to bottom, #0f0c29, #302b63, #24243e)' },
    { id: 'deep-space', name: 'Deep Space', value: 'radial-gradient(circle at bottom left, #1b2735 0%, #090a0f 100%)' },
    { id: 'matrix', name: 'The Matrix', value: 'linear-gradient(0deg, #000000 0%, #0a1a0a 100%)' },
];
    ...
    persist(
        (set, get) => ({
            ...
            getWallpaperValue: () => {
                ...
                return WALLPAPERS.find(w => w.id === wallpaperId)?.value || WALLPAPERS[0]?.value || '';
            },

            getAccentColorValue: () => {
                const { accentColorId } = get();
                return ACCENT_COLORS.find(c => c.id === accentColorId)?.value || ACCENT_COLORS[0]?.value || '';
            },
        }),
        {
